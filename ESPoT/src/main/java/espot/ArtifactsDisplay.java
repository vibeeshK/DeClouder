...
public abstract class ArtifactsDisplay {
	...
	public void displayArtifact() {
		...
		SysTrayHanlder sysTray =  new SysTrayHanlder(commonUIData.getESPoTDisplay(), mainShell, displayTitle, commonUIData.getCommons().backgroundImagePathFileName);
		sysTray.displayTray();
	...
	private void displayContent() {
		...
		for (int screenRowNo = 0; screenRowNo < tableItems.length; screenRowNo++) {
			...
			editor = new TableEditor(table);
			Text textERLName = new Text(table, SWT.READ_ONLY);
			textERLName.setText(displayArtifact.artifactKeyPojo.artifactName);
			editor.grabHorizontal = true;
			editor.setEditor(textERLName, tableItem, columnCount++);
